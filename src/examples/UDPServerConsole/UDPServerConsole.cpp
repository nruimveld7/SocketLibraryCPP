...
int main(int argc, char* argv[]) {
	...
	Configure();
	if(!g_spawned) {
		std::cout << "Between messages, enter '/m' or '/c' to modify or close the socket respectively." << std::endl
			<< std::endl;
	}
...
void InputHandler(std::string input) {
    ...
    while(true) {
      ...
      std::string candidateIP = targetIP;
      int inlinePort = -1;
      size_t colonPos = targetIP.find(':');
      if(colonPos != std::string::npos) {
        candidateIP = targetIP.substr(0, colonPos);
        std::string portStr = targetIP.substr(colonPos + 1);
        if(portStr.empty()) {
          std::cout << "Error: Missing port after ':'." << std::endl;
          continue;
        }
        try {
          inlinePort = std::stoi(portStr);
        } catch(const std::exception& e) {
          std::cout << "Error: " << e.what() << ". Please try again!" << std::endl;
          continue;
        }
        if(!Socket::CheckPort(inlinePort)) {
          std::cout << "Error: Invalid port. Please try again!" << std::endl;
          continue;
        }
      }
      if(Socket::CheckIP(candidateIP)) {
        targetIP = candidateIP;
        if(inlinePort != -1) {
          targetPort = inlinePort;
          portProvided = true;
        }
        break;
      }
...
std::string Trim(const std::string& str) {
	...
	auto end = std::find_if_not(str.rbegin(), str.rend(), [](unsigned char c) { return std::isspace(c); })
		.base();
